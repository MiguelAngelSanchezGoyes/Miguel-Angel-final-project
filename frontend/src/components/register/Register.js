...
export default function Register() {
  ...
  const handleSubmit = (ev) => {
    ev.preventDefault();
    dispatch(createUser(registerState));
    console.log(registerState);

    if (registerState.typeUser === 'user') {
      setRegisterState({ name: '', email: '', passwd: '', typeUser: '' });
      navigate('/profile');
    } else if (registerState.typeUser === 'professional') {
      setRegisterState({ name: '', email: '', passwd: '', typeUser: '' });
  ...
  const template = (
    <form className="login-form" onSubmit={handleSubmit}>
      ...
      <div className="flex-row">
        <label className="lf--label" htmlFor="user">
          <svg x="0px" y="0px" width="15px" height="5px">
            <g>
              <path
                fill="#B1B7C4"

...
import { verifyUser } from '../../redux/user/action-creator';
import './Login.scss';
...
export default function Login() {
  ...
  const handleSubmit = (ev) => {
    ev.preventDefault();
    dispatch(verifyUser(loginState));

    if (loginState.typeUser === 'user') {
      setLoginState({ name: '', email: '', passwd: '', typeUser: '' });

      navigate('/profile/');
    } else if (loginState.typeUser === 'professional') {
      setLoginState({ name: '', email: '', passwd: '', typeUser: '' });

      navigate('/professionalProfile');
    } else {
      Error('You must choose between user or professional');
    }
  };
  ...
  const template = (
    <form className="login-form" onSubmit={handleSubmit}>
      ...
      <div className="flex-row">
        <label className="lf--label" htmlFor="typeUser">
        ...
        <input
          ...
          type="text"
          value={loginState.typeUser}
          required
          onChange={(ev) => handleChange(ev, 'typeUser')}
        />
      </div>
      <div className="flex-row">

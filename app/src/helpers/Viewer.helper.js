...
import { getViewerProperties } from '../redux/viewer/action-creator';
import { store } from '../redux/store';

import Client from '../components/viewer/ViewerClient';

let viewer;
const getToken = { accessToken: Client.getaccesstoken() };
let explodeScale = 0;
let startExplosion = null;
let explosionReq;
let isExploding = false;
let outwardExplosion = true;
        ...
        viewer.addEventListener(
          ...
          debounce(() => {
            ...
            getModelProperties()
              ...
              .catch(() => store.dispatch(getViewerProperties([])));
          }),
          200
        );
...
export function explodeMotion(timestamp) {
  ...
  const progress = timestamp - startExplosion;
  startExplosion = timestamp;
  const explodeStep = 0.0002 * (progress || 0);
...
export function rotateMotion(timestamp) {
  ...
  const progress = timestamp - startRotation;
  startRotation = timestamp;
  const rotateStep = 0.0005 * (progress || 0);
  // get the up axis
  const worldUp = viewer.navigation.getWorldUpVector();
  // get the current position
  const pos = viewer.navigation.getPosition();
  // copy that position
  const position = new THREE.Vector3(pos.x, pos.y, pos.z);
  // set the rotate axis
  const rAxis = new THREE.Vector3(worldUp.x, worldUp.y, worldUp.z);
  const matrix = new THREE.Matrix4().makeRotationAxis(rAxis, rotateStep);
